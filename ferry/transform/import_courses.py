...
exactly_identical_crns = [
    ...
    ("202503", (11604, 11605)),  # MUS 506
    ("202601", (23900, 23902)),  # MUS 506
    ("202503", (11607, 11606)),  # MUS 606
    ("202601", (23903, 23904)),  # MUS 606
    ("202503", (16887, 16889)),  # MUS 706
    ("202601", (22784, 26504)),  # MUS 540
...
def aggregate_locations(
    ...
    for meetings in courses["meetings"]:
        for meeting in meetings:
            ...
            try:
                ...
                if (
                    parsed_location["code"] is not None
                    and str(parsed_location["code"]).strip() != ""
                ):
    ...
    course_meetings = (
        course_meetings.groupby(
            ["course_id", "start_time", "end_time", "_building_code", "_room"],
            dropna=False,
        )
...
def import_courses(data_dir: Path, seasons: list[str]) -> CourseTables:
    ...
    with warnings.catch_warnings():
        ...
        listings = pd.concat(
            all_imported_listings, axis=0, ignore_index=True, copy=False
        )
