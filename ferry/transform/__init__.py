...
from .import_demand import import_demand
...
async def transform(data_dir: Path) -> dict[str, pd.DataFrame]:
    ...
    listing_demand_table, course_demand_table = import_demand(
        data_dir, course_seasons, course_tables["listings"]
    )

    # Force garbage collection after importing demand statistics
    gc.collect()
    ...
    all_tables = {
        ...
        **course_tables,
        **eval_tables,
        "listing_demand_statistics": listing_demand_table,
        "course_demand_statistics": course_demand_table,
    }
    ...
    for table_name, db_table in [
        ...
        ("listing_demand_statistics", database.ListingDemandStatistics.__table__),
        ("course_demand_statistics", database.CourseDemandStatistics.__table__),
    ]:
        if table_name == "listing_demand_statistics":
            # id is auto-generated by postgres on insert (unlike other
            # tables) since this table is upserted incrementally across many
            # runs rather than recreated each time - reusing pandas'
            # per-run sequential ids here would collide with real ids
            # already committed to the database from earlier runs.
            db_columns = [
                column.key for column in db_table.columns if column.key != "id"
            ]
            current_table = all_tables[table_name]
            available_columns = [
                col for col in db_columns if col in current_table.columns
            ]
            all_tables[table_name] = current_table[available_columns]
        elif table_name == "course_demand_statistics":
            # No surrogate id - primary key is the natural (course_id, date)
            # pair, and this table is fully derived from
            # listing_demand_statistics, never targeted by other FKs.
